...
## G5f — audition 2, Part A: a quota, a ruling, then a landed 10/21

`docs/briefs/g5f-audition-2.md`. Full narrative in `docs/MCGROTS-VOICE.md` §
"Audition 2, 2026-08-15 — Part A (Gemini) landed, from Dan's own briefs" —
this entry is the verification account: what was run, what broke, what
changed, and why nothing was invented to look more finished than it is.

**The rig gained a `gemini`/`fal`/`self-test` subcommand split.** `self-test`
is unchanged from G5e. `fal` is G5e's flow, relocated, plus a cheap probe call
before spending on the batch (unexercised this session — Part B was paused
before it started; see below). `gemini` is new: twelve solo lines
(`generated/mcgrots-dialogue.json`, `mcgrot-01`..`12`) on Algenib, plus a
three-voice sweep (Orus, Fenrir, Gacrux) on the three lines that overlap the
FAL audition set, for cross-engine comparison on identical content.

**Real run, first version: 9 ok, 12 failed, ~$0.0100 spent.** Not a bug —
`generativelanguage.googleapis.com`'s `gemini-2.5-flash-tts` free tier caps at
**10 requests/day/project/model**
(`quotaId: GenerateRequestsPerDayPerProjectPerModel-FreeTier`, read from the
raw error body via a direct `curl`, not inferred from the summarised
message — the summarised 429 text alone would have read as an ordinary rate
limit and invited a retry loop that could only fail identically). Confirmed
this quota is shared with `scripts/daily-tts.sh`'s own cron against the same
key, which is why fewer than 10 of this session's own calls succeeded.
**Stopped, not retried further, per the same discipline `generate-tts.mjs`
already documents for its own daily quota** — retrying a daily cap is not a
transient failure to back off from.

**Then, before Part B started: Dan ruled the director's briefs must be
authored by him, not generated by this script.** The code that assembled a
brief from `docs/MCGROTS-VOICE.md`'s prompt + the dialogue file's scene was
removed outright, not left dead. `runGemini()` now reads
`scripts/mcgrots-voice-briefs/mcgrot-NN.txt` per line, sends it verbatim, and
skips (not errors) any line with no file yet — printed as MISSING in
`--dry-run`, excluded from the cost estimate, and excluded from what a real
run would spend. Re-run `node scripts/mcgrots-voice-audition.mjs gemini
--dry-run` after the change: the four brief files that existed at that point
(`mcgrot-01`..`04`) showed as real, sized entries; the other eight showed
MISSING and were excluded from the estimate — the split is real, not
cosmetic. Rendering stayed paused until Dan explicitly lifted it and pointed
at the twelve briefs committed at `55db0d2` — file presence on disk was not
treated as permission on its own.

**All twelve briefs read before spending, per the resume instruction — no
issues found.** `mcgrot-03`, `01`, `07` and `12` opened by hand: each carries
Audio Profile / Scene / Director's Notes / script in the same shape as
`3c6b637b.txt`, the line text and delivery bracket match
`generated/mcgrots-dialogue.json` exactly, and the CRITICAL verbatim
instruction is present in every one checked. Nothing reported back to Dan.

**Resumed, quota hit again, stopped deliberately rather than ground through
it.** `--dry-run` after the resume showed all 21 jobs with real brief text (no
MISSING). The real run: probe OK, four jobs skipped (already on disk from the
earlier attempt), `mcgrot-05` rendered, then `mcgrot-06` and `mcgrot-07` each
burned a full 4-attempt/121s backoff cycle on the same `RESOURCE_EXHAUSTED`
before failing — proving the daily cap, not a per-minute one, had reasserted
itself after only two more successful calls. The run was killed
(`TaskStop`) rather than left to spend roughly 20 more minutes grinding
through the remaining 9 jobs for a guaranteed identical result. No partial or
zero-byte file resulted from the kill — the in-flight `mcgrot-12` job had not
reached its write when stopped, and has no manifest entry, so a future rerun
will retry it cleanly.

**Landed: 10/21, ~$0.0098 real spend (measured from actual rendered
durations, not estimated).** `mcgrot-01`..`05`, `08`..`11` on Algenib, plus
`mcgrot-01` on Orus. The remaining 11 — `06`, `07`, `12` on Algenib, and the
rest of the sweep — are `failed` in the manifest with their real 429 bodies,
not silently dropped, and need only the daily quota to reset; no code change
is pending on them.

**A false claim caught before it shipped, in a comment, not code.** While
this section was being written, `FAL_ENGINES.minimax`'s comment described a
`--reuse-voice=` flag "below" that reuses a captured `custom_voice_id` through
a speech endpoint — Part B's actual scope, per the brief. It was never built;
Part B was paused before it started. Caught on a self-review pass before
commit and corrected in place to say plainly that design-once-reuse is NOT
YET BUILT. Recorded here because this is exactly the failure mode the
verification discipline exists to catch, and it happened in this session's
own comment, not in someone else's code.

**Resumability's known gap (flagged in the review above) closed for both
engines while touching this code anyway.** `existsSync` alone treated a
zero-byte or truncated file as done. `isRendered()` now requires `size > 0`
too. Not fault-injected this session — no code path currently writes a
zero-byte file on the real path (the review above already established that
for FAL; Gemini's write is `pcmToWav` → `ffmpeg`, which errors rather than
emptying a file on a bad input) — recorded as a robustness fix against a
future truncated-download case, not a verified-red-then-green gate.
