...
const REGIONS = ['boot', 'camera', 'statue', 'anchors', 'van', 'pomple', 'mcgrot', 'seat', 'rota', 'audio', 'visit', 'style', 'dialogue', 'beats', 'taxman', 'ambience', 'valance'];
...
try {
  ...
  if (needsBrowser) {
    ...
    // ------------------------------------------------------------ valance ---
    // G7o (docs/briefs/g7o-van-valance.md, docs/MCGROTS-ROADMAP.md § 10 F24).
    // McGrot stands at road level inside the van's plan footprint, so his legs
    // occupy the same space as the van's underside. `van.js` now builds a
    // valance — a skirt from the floor (0.62 m) down toward the road, between
    // the wheels — named `van-valance`, toggled by `.visible`.
    //
    // THIS IS A CONCEALMENT, NOT A CORRECTION, and the gate says so too: it
    // proves the overlap is hidden from the camera, not that the ground planes
    // agree. Own navigation (skinned body: the capsule has no rig detail to
    // read a leg silhouette off), so this does not inherit whatever body an
    // earlier region left the page on — the exact `beats` bug this project has
    // already paid for once (AGENTS.md, "must pass ON ITS OWN"). Clock pinned
    // to 980 throughout — rota.js's own reference for "no reader present"
    // (see the mcgrot region's F22 comment) — so the rota reader never
    // contaminates a measurement aimed at the van, not at him.
    if (wants('valance')) {
      await page.goto(`http://127.0.0.1:${port}/mcgrots.html?body=skinned&archetype=rab`,
        { waitUntil: 'load' });
      await page.waitForFunction(() => !!window.__mcgrotsDebug, null, { timeout: 15000 });
